...
We will write $P_\phi$ for the P-gate, which dependes on a parameter $\phi$, that maps qubits as follows 
...
Similarly, we can plug the QFT-circuit above into the recursive formula @eq-rec-QFT and (after expanding and simplyfying) get $QFT_4$ in terms of H, CP, and swap gates:
...
In doing so, it will coninue to use the notation from @sec-CPC by writing
$$
|a\rangle=|a_n\rangle\otimes|a'\rangle.
$${#eq-aap}
Following @eq-rec-QFT, we first apply the Hardaman gate (which is $QFT_1$) to the first (top) qubit:
$$
...
$$
Next, (according to @eq-rec-QFT) we apply the QFT-gate to the last (bottom) $n$ qubits using @eq-closed-QFT:
$$
\dfrac{1}{\sqrt{2}}\sum_{0\leq b_0\leq 1}e^{\frac{\pi i b_0 a}{2^n}}|b_0\rangle\otimes|a'\rangle
\mapsto
